...
void gui::render_main_tab()
{
	if (ImGui::BeginTabItem("Main")) 
	{
		static bool infinite_dodge{};
		if (ImGui::Checkbox("Infinite dodge", &infinite_dodge)) 
		{
			if (infinite_dodge) 
			{
				DWORD currProt, temp;
				VirtualProtect(g_pointers->m_infinity_dodge, 1, PAGE_EXECUTE_READWRITE, &currProt);
				memset(g_pointers->m_infinity_dodge, 0xF0, 1);
				VirtualProtect(g_pointers->m_infinity_dodge, 1, currProt, &temp);
			}
			else 
			{
				DWORD currProt, temp;
				VirtualProtect(g_pointers->m_infinity_dodge, 1, PAGE_EXECUTE_READWRITE, &currProt);
				memset(g_pointers->m_infinity_dodge, 0xF1, 1);
				VirtualProtect(g_pointers->m_infinity_dodge, 1, currProt, &temp);
			}
		}
